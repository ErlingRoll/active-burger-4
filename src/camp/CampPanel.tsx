...
import type { CampJobDefinition, CampJobId } from '../content/camp/CampTypes'
...
export function CampPanel({
  ...
  useEffect(() => {
    ...
    let cancelled = false
    void Promise.all([service.loadState(), characterService.loadCharacters()])
      .then(([campState, collection]) => {
        if (!cancelled) {
          setState(campState)
        ...
        <>
          ...
          <ul className="camp-jobs">
            {ALL_CAMP_JOB_DEFINITIONS.map((job) => {
              ...
              const level = state.buildings.find((entry) => entry.buildingId === job.buildingId)?.level ?? 1
