...
import type { Rarity as RarityValue } from '../rarity/Rarity'

/**
 * Upgrade schema types and the pure predicates over them.
 *
 * These are held apart from `Upgrades.ts` so `game-config/skill-upgrades.ts`
 * and `game-config/synergies.ts` can be typed by them without importing a
 * module that re-exports their own tuning data back. This module imports only
 * other schema modules, which is what makes the dependency from `game-config/`
 * into `content/` one-directional.
 */
