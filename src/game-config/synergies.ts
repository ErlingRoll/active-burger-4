...
export const SYNERGY_UPGRADES: readonly SynergyUpgradeDefinition[] = [
  ...
  createSynergyUpgrade(
    ...
    'Fractured Echo',
    'Each qualifying Basic Attack critical launches one additional Critical Spellstrike replay of its focused skill. Critical Spellstrike base cooldown becomes 1.5 seconds before cooldown reduction.',
    'Double replay; 1.5s base cooldown before CDR',
    [CRITICAL_SPELLSTRIKE_SKILL_ID, MIRRORCAST_SKILL_ID],
  ),
  createSynergyUpgrade(
    'synergy-critical-spellstrike-razorwire',
    'Tension Cascade',
    'Each Critical Spellstrike replay extends only your newest active Razorwire by 2 seconds, capped at its existing maximum duration.',
    'Replays extend newest Razorwire (cap respected)',
    [CRITICAL_SPELLSTRIKE_SKILL_ID, RAZORWIRE_SKILL_ID],
  ),
  createSynergyUpgrade(
    'synergy-critical-spellstrike-prism-halo',
    'Critical Spectrum',
    'Each Critical Spellstrike replay immediately fires one existing Prism Halo shard while the Halo is active.',
    'Replays fire one Prism Halo shard',
    [CRITICAL_SPELLSTRIKE_SKILL_ID, PRISM_HALO_SKILL_ID],
  ),
  createSynergyUpgrade(
