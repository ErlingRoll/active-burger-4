...
describe('Game', () => {
  ...
  it('equips each class with its authored starter weapon', () => {
    expect(createGame({ seed: 101, characterClassId: 'knight' }).state.player.equipment?.weapon)
      .toMatchObject({ itemId: 'knight-training-sword' })
    expect(createGame({ seed: 102, characterClassId: 'ranger' }).state.player.equipment?.weapon)
      .toMatchObject({ itemId: 'ranger-training-bow' })
    expect(createGame({ seed: 103, characterClassId: 'necromancer' }).state.player.equipment?.weapon)
      .toMatchObject({ itemId: 'necromancer-bone-staff' })
    expect(createGame({ seed: 104, characterClassId: 'frost-warden' }).state.player.equipment?.weapon)
      .toMatchObject({ itemId: 'frost-warden-training-wand' })
    expect(createGame({ seed: 105, characterClassId: 'ashen-alchemist' }).state.player.equipment?.weapon)
      .toMatchObject({ itemId: 'ashen-alchemist-training-staff' })
    expect(createGame({ seed: 106, characterClassId: 'war-shepherd' }).state.player.equipment?.weapon)
      .toMatchObject({ itemId: 'war-shepherd-training-sword' })
    expect(createGame({ seed: 107, characterClassId: 'riftwalker' }).state.player.equipment?.weapon)
      .toMatchObject({ itemId: 'ranger-training-bow' })
    expect(createGame({ seed: 108, characterClassId: 'bloodweaver' }).state.player.equipment?.weapon)
      .toMatchObject({ itemId: 'ashen-alchemist-training-staff' })
  })
