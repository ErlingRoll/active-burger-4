-- Every 10th completed Infinite Abyss floor guarantees at least an epic loot
-- box, with a 5% chance of that guaranteed box being legendary instead.
-- Rewritten whole from the version in 20260912140000_add_camp_anchor_and_smokehouse.sql,
-- with only the rarity resolution changed; the rift shard grant is untouched.
...
begin
  ...
  if mod(v_completed_floor, 10) = 0 then
    ...
    -- milestone bonus roll bit-for-bit.
    v_rarity := case
      when mod(v_seed # 1539272357, 10000) < 500 then 'legendary'
      else 'epic'
    end;
  else
