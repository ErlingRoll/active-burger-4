-- start_dungeon_run exists twice: once with p_max_floor integer and once
-- with p_max_floor bigint. PostgREST resolves an RPC by name, and a JSON
-- number satisfies both, so every call to start a run fails with "Could not
-- choose the best candidate function".
--
-- The widening migration dropped the integer overload before creating the
-- bigint one, so a history applied in order leaves one function. An earlier
-- migration's `create or replace ... p_max_floor integer` applied after it
-- adds the integer overload back rather than replacing anything, because a
-- different parameter type is a different function. Rather than derive the
-- survivor from whatever the database holds, this migration carries the
-- definition itself: both overloads go, the bigint one is created from the
-- text below, and the file ends by checking that exactly one remains.
--
-- The body is the one the history arrives at: the fish-meal resolution from
-- 20260905210000, the qualified reservation status check from
-- 20260906200000, and the bigint max_floor from 20260906210000.
...
begin
  ...
  where pronamespace = 'public'::regnamespace
    and proname = 'start_dungeon_run';
  if v_overload_count <> 1 then
    raise exception 'Expected exactly one start_dungeon_run function, found %.', v_overload_count;
  end if;
end;
$$;
