-- Qualify Champion table columns because RETURNS TABLE creates PL/pgSQL
-- variables with the same names, including "id".

create or replace function public.revive_champion_with_fish(
  p_operation_id text,
  p_champion_id text,
...
begin
  ...
  from public.champion_revival_operations as operations
  ...
  where champions.id = p_champion_id
    and champions.profile_id = v_profile_id;
  update public.champion_revival_operations as operations
