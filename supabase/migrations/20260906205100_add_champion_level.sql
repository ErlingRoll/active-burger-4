-- Persist the character level in Champion build snapshots and backfill
-- Champions created before this migration from their source run checkpoint.

with latest_snapshots as (
  select distinct on (snapshots.run_id)
    snapshots.run_id,
    snapshots.payload
  from public.dungeon_run_snapshots as snapshots
  order by snapshots.run_id, snapshots.id desc
)
update public.champions as champions
set build = champions.build || jsonb_build_object(
  'level',
  case
    when (latest_snapshots.payload -> 'gameState' -> 'player' ->> 'level')
      ~ '^[0-9]+$'
    then greatest(
      1,
      (latest_snapshots.payload -> 'gameState' -> 'player' ->> 'level')::integer
    )
    else 1
  end
)
from latest_snapshots
where champions.source_run_id = latest_snapshots.run_id
  and jsonb_typeof(champions.build) = 'object'
  and not (champions.build ? 'level');
