...
begin
  ...
  -- Keep server-side reward ordering in sync with the content catalog.
  for v_modifier_id in
    ...
    order by case modifiers.value
      when 'swarming' then 1
      when 'fast-start' then 2
      when 'juggernauts' then 3
      when 'glass-world' then 4
      when 'shorter-minute' then 5
      when 'elite-invasion' then 6
    end
  loop
    v_multiplier := v_multiplier * (
      1 + (
        case v_modifier_id
          ...
          when 'shorter-minute' then 1.15
          when 'elite-invasion' then 1.20
          when 'fast-start' then 1.08
          else 1
        end - 1
      ) * power(0.75::numeric, v_modifier_index)
    );
    v_modifier_index := v_modifier_index + 1;
  end loop;
  v_multiplier := least(v_multiplier, 1.50);
