drop function if exists public.get_essence_leaderboard();

create function public.get_essence_leaderboard()
returns table (
  profile_id uuid,
  display_name text,
...
as $$
  with ranked_entries as (
    select
      ...
      coalesce(nullif(trim(profiles.display_name), ''), 'Anonymous player') as display_name,
