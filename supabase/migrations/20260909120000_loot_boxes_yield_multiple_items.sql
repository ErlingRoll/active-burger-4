-- Loot boxes give up more than one item.
--
-- `open_loot_box` rolled once and returned a single row, and the interface
-- around it opened a box, showed one thing in a corner toast, and moved on.
-- The opening is now a moment the player watches, and a moment is worth
-- watching in proportion to what falls out of it, so a rare box draws twice,
-- an epic three times and a legendary four.
--
-- Every draw is rolled separately against the same table, which means a box
-- can give the same thing twice. That is deliberate: the alternative is
-- drawing without replacement, which quietly guarantees variety and makes the
-- rarest entries far more common than their published odds say.
--
-- The recorded result becomes a list. `loot_box_openings` keeps its singular
-- `result_*` columns so that nothing reading them breaks and so the unique
-- constraint on (profile_id, operation_id) still does the idempotency work;
-- they now describe the first draw. The full list lives in the child table
-- below, and existing rows are backfilled into it so a replayed operation from
-- before this migration still answers with what it gave the first time.

create table if not exists public.loot_box_opening_items (
  id bigint generated always as identity primary key,
  opening_id bigint not null
    references public.loot_box_openings (id) on delete cascade,
  ...
  unique (opening_id, draw_index)
);

create index if not exists loot_box_opening_items_opening_idx
  on public.loot_box_opening_items (opening_id, draw_index);

alter table public.loot_box_opening_items enable row level security;

drop policy if exists "Loot-box opening items are readable by their owner"
  on public.loot_box_opening_items;

create policy "Loot-box opening items are readable by their owner"
on public.loot_box_opening_items for select to authenticated
using (
  exists (
    select 1
    from public.loot_box_openings as openings
    where openings.id = loot_box_opening_items.opening_id
      and openings.profile_id = (select auth.uid())
  )
);

-- Every opening that happened before this migration was a single draw.
...
select
  openings.id,
  1,
  openings.result_item_instance_id,
  openings.result_definition_id,
  openings.result_quantity,
  openings.result_metadata
from public.loot_box_openings as openings
where not exists (
  select 1
  from public.loot_box_opening_items as items
  where items.opening_id = openings.id
);
...
begin
  ...
  v_drop_table := case v_box_rarity
    ...
    when 'uncommon' then '[["river-minnow",400],["revival-koi",650],["river-worm",850],["glow-grub",950],["starter-fishing-rod",1000]]'::jsonb
    when 'rare' then '[["river-minnow",250],["revival-koi",500],["river-worm",750],["glow-grub",900],["moonwater-lure",950],["starter-fishing-rod",1000]]'::jsonb
    when 'epic' then '[["river-minnow",200],["revival-koi",400],["river-worm",650],["glow-grub",850],["moonwater-lure",950],["starter-fishing-rod",1000]]'::jsonb
    else '[["river-minnow",100],["revival-koi",250],["river-worm",450],["glow-grub",700],["moonwater-lure",900],["starter-fishing-rod",1000]]'::jsonb
  end;
